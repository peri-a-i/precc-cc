...
NO_VERIFY=""
# Companion tools (lean-ctx, rtk, nushell) are installed by default because
# they contribute the bulk of token savings in live measurements (lean-ctx
# is the top-performing mode). Use --no-extras to skip companions entirely.
#
# cocoindex-code (AST-driven semantic code search) is *opt-in* via --ccc:
# in usage data its CCC pillar contributed near-zero tokens vs. RTK / lean-ctx
# / mined-preventions, while it is by far the heaviest companion to install
# (Python pre-release deps via pipx / uv). Users who want it can pass --ccc
# at install time or run `precc update --extras --ccc` later.
EXTRAS=1
EXTRAS_ONLY=""
WITH_CCC=""
FORCE=""
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --no-extras)
            ...
            ;;
        --force)
            # Bypass the "already installed → skip" guard in the companion
            # installers. Used by `precc update --extras` to upgrade pinned
            # versions when newer releases are available upstream.
            FORCE=1
            shift
            ;;
        --ccc)
            # Opt into the cocoindex-code (semantic code search) install.
            # No longer in the default companion bundle — see the comment
...
# Hoisted out of the EXTRAS_ONLY conditional below so both the precc-binary
# install branch *and* the companion installers (which reference $TMP for
# tarball downloads) get a valid temp dir. Previously --extras-only set the
# flag but skipped the block where TMP was defined, so companion installs
# died with `TMP: unbound variable`.
TMP="$(mktemp -d)"
trap 'rm -rf "${TMP}"' EXIT
...
install_lean_ctx() {
    if command -v lean-ctx &>/dev/null; then
        local current_ver
        current_ver="$(lean-ctx --version 2>/dev/null)"
        if [[ -z "${FORCE}" ]]; then
            echo "  lean-ctx already installed: ${current_ver}"
            return 0
        fi
        echo "  lean-ctx currently ${current_ver} — upgrading (--force)..."
    fi
...
install_rtk() {
    if command -v rtk &>/dev/null; then
        local current_ver
        current_ver="$(rtk --version 2>/dev/null | head -1)"
        if [[ -z "${FORCE}" ]]; then
            echo "  RTK already installed: ${current_ver}"
            return 0
        fi
        echo "  RTK currently ${current_ver} — upgrading (--force)..."
    fi
...
# ---------------------------------------------------------------------------
# Companion tools (rtk, lean-ctx, nushell) are installed by default because
# they contribute the bulk of measured token savings. Use --no-extras to
# skip them entirely. cocoindex-code is opt-in via --ccc — see top of file.
# ---------------------------------------------------------------------------
if [[ -n "${EXTRAS}" ]]; then
    echo ""
    ...
    if [[ -n "${WITH_CCC}" ]]; then
        echo "  - cocoindex-code:  AST-driven semantic code search (--ccc)"
    fi
    echo "  (Pass --no-extras to skip; --ccc to add cocoindex-code)"
    echo ""
...
install_nushell() {
    if command -v nu &>/dev/null; then
        local current_ver
        current_ver="$(nu --version 2>/dev/null)"
        if [[ -z "${FORCE}" ]]; then
            echo "  Nushell already installed: ${current_ver}"
            return 0
        fi
        echo "  Nushell currently ${current_ver} — upgrading (--force)..."
    fi
...
install_cocoindex_code() {
    if command -v ccc &>/dev/null; then
        local current_ver
        current_ver="$(ccc --version 2>/dev/null || echo 'unknown')"
        if [[ -z "${FORCE}" ]]; then
            echo "  cocoindex-code already installed: ${current_ver}"
            return 0
        fi
        echo "  cocoindex-code currently ${current_ver} — upgrading (--force)..."
    fi
    ...
    if command -v pipx &>/dev/null; then
        echo "  Using pipx..."
        # pipx has its own "already installed" guard that ignores our top-level
        # FORCE flag; pass --force through so --extras mode actually upgrades.
        local pipx_force=""
        [[ -n "${FORCE}" ]] && pipx_force="--force"
        timeout 240 pipx install ${pipx_force} --pip-args='--pre' cocoindex-code \
...
if [[ -n "${WITH_CCC}" ]] || (command -v ccc &>/dev/null && [[ -n "${FORCE}" ]]); then
    # Install fresh when --ccc is explicit. Also upgrade-in-place when ccc
    # is already on PATH and --force is set, so existing users don't lose
    # their cocoindex install on `precc update --extras`. Plain installs
    # without --ccc skip cocoindex entirely (zero contribution in measured
    # data, heaviest companion to install).
    install_cocoindex_code
    wire_mcp_cocoindex
fi
...
if command -v ccc &>/dev/null; then
    echo "cocoindex-code is available. Run 'precc index' in your project to enable AST-based semantic search."
fi
