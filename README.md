...
- **Stop wasting tokens on wrong-directory errors.** Claude runs `cargo build` (or `git status`, `npm test`) from a parent directory; instead of letting it fail and burn tokens on the error and a retry, PRECC detects the missing `Cargo.toml`/`.git` and prepends the correct `cd` so the command works the first time.
- **Compress verbose command output before it reaches the model.** A `cargo test`, `npm install`, or `find` produces hundreds of lines; PRECC compresses that output pre-execution (delegating to `lean-ctx`/`nushell`/RTK when installed) — measured at ~22.6 % token savings on Bash output, with the hook completing in a few milliseconds.
- **Don't read Office/PDF files as text.** When Claude tries `cat deck.pptx` or `head report.pdf`, PRECC flags that dumping binary bytes wastes thousands of tokens and points it at the right extractor (`markitdown`, `pdftotext`, or a CSV export for spreadsheets); it also nudges away from rasterizing every page of a large PDF.
- **Knowledge work in Claude Cowork.** The plugin installs and runs entirely offline in Cowork's sandbox (prebuilt binary bundled, no network), so the same command correction and document guards apply to slides/spreadsheets/PDF tasks — not just coding.
- **See and track what you're saving.** `/precc savings` reports measured token savings; `/precc predict` estimates the token cost of a planned multi-step task and records actuals to improve accuracy over time; the status bar shows live per-session savings.
- **Self-improving over a project.** PRECC mines your session logs for recurring failure→fix patterns and turns them into deterministic command rewrites, so the same mistake isn't repeated — no extra LLM calls, all local.
